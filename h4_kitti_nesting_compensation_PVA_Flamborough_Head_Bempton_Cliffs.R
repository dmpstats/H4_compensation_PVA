#' =============================================================================================================
#' ~~ Purpose: NEPVA model for Kittiwakes in Flamborough Head and Bempton Cliffs 
#'          
#' ~ Author: BARC
#' 
#' ~ Created: 14/05/2021
...
pva_flam_bemp <- nepva.simplescenarios(
  model.envstoch = m_es,
...
#
# Extract draws for chicks for baseline scenario (shape = (nyears, nsim))

philopatry_rate <- c(0.75, 0.5, 0.2)

flam_bemp_migrants_wide <- list()

for(i in 1:length(philopatry_rate)){
  flam_bemp_migrants_wide[[i]] <- as_tibble(pva_flam_bemp$raw$nbyage[1, 1, , , 1] * (1 - philopatry_rate[i]),
                                            .name_repair = "unique") %>%
    mutate(spa = spa_name,
...
flam_bemp_migrants_wide <- bind_rows(flam_bemp_migrants_wide) %>%
  arrange(fledged_year, recruited_year, desc(phylopatry_rate))



# Convert to long, which is needed for plotting
flam_bemp_migrants_long <- flam_bemp_migrants_wide %>%
  pivot_longer(
...
# write-out data
write_rds(flam_bemp_migrants_wide, path = "outputs/nesting/pva_Flamb_Head_Bempt_Cliffs_migrants.rds", 
          compress = "gz")



# flam_bemp_migrants_long <- as_tibble(pva_flam_bemp$raw$nbyage[1, 1, , , 1], 
#                                     .name_repair = "unique") %>%
#   mutate(spa = spa_name,
#          fledged_year = sim_years,
#          .before = 1) %>%
#   pivot_longer(cols = -c(spa, fledged_year), names_to = "sim", values_to = "n_chicks") %>%
#   mutate(
#     recruited_year = fledged_year + breedAge,
#     adultMigrants_phil75 = n_chicks * (1-0.75),
#     adultMigrants_phil50 = n_chicks * (1-0.50), 
#     adultMigrants_phil20 = n_chicks * (1-0.20),
#     sim = as.integer(str_replace(sim, pattern = "...", ""))
#     ) %>%
